...
public class BaseSetUp {
	 ...
	 * reports; public static ExtentTest test;
	 */
	...
	public static void androidPhoneConfiguration() {
		...
		try {
			...
			System.out.println(driver
					.findElementById("com.jch_hitachi.aircloudglobal:id/text_view_confirm_dialog_sub_title").getText());
		}
		driver.manage().timeouts().implicitlyWait(20, TimeUnit.SECONDS);
	}

	/*
	 * @BeforeClass public void extentConfig() { htmlReporter = new
	 * ExtentHtmlReporter(System.getProperty("user.dir")+
	 * "/test-output/AndroidTestReport.html"); reports = new ExtentReports();
	 ...
	 * CaptureScreenShot.getScreenShotForExtent(result.getName());
	 * test.log(Status.FAIL, MarkupHelper.createLabel(result.getName() +
	 * " Test FAILED: ", ExtentColor.RED)); test.fail(result.getThrowable());
	 * test.fail("Snapshot below: " +
	 * test.addScreenCaptureFromPath(screenShotPath)); } else if (result.getStatus()
	 * == ITestResult.SUCCESS) { test.log(Status.PASS,
	 * MarkupHelper.createLabel(result.getName() + " Test PASSED",
	 * ExtentColor.GREEN)); } else { test.log(Status.PASS,
	 * MarkupHelper.createLabel(result.getName() + " Test SKIPPED",
	 * ExtentColor.ORANGE)); } }
	 * 
	 * @AfterClass public void extentFlushTest() { reports.flush(); }
	 */
